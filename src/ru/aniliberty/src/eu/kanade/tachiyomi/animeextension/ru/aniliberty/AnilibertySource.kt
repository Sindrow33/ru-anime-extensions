...
class AnilibertySource :
    ...
    override fun searchAnimeRequest(page: Int, query: String, filters: AnimeFilterList): Request {
        ...
        val url = "$apiUrl/anime/catalog/releases".toHttpUrl().newBuilder()
            ...
            .apply {
                if (genre.isNotEmpty()) addQueryParameter("f[genres]", genre)
            }
            .build()
        return GET(url.toString(), headers)
    }
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        ...
        val ep = release.episodes.firstOrNull { it.id == episodeId } ?: return emptyList()
    ...
    private fun Release.toSAnime(): SAnime = SAnime.create().apply {
        ...
        description = buildString {
            year?.let { append(": $it\n") }
            type?.description?.let { append(": $it\n") }
    ...
    private class GenreFilter : AnimeFilter.Select<String>("", GENRES.map { it.first }.toTypedArray()) {
